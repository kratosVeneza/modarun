...
// Componente separado para a barra visual.
// Isso é necessário porque template literals dentro de style={{}} em JSX
// causam erros de tipo no TypeScript strict. Ao extrair para um componente,
// o valor de width vira uma variável string simples, sem problema.
function BarraZona({ pct, cor }: { pct: number; cor: string }) {
  const largura = String(Math.round(pct * 100)) + "%";
  const fundo = "linear-gradient(90deg, " + cor + "40, " + cor + ")";
  return (
    <div className="h-full rounded-full" style={{ width: largura, background: fundo }} />
  );
}
...
export default function CalculadoraFCPage(): React.JSX.Element {
  ...
  const [resultado, setResultado] = useState<null | {
    ...
    zonas: {
      zona: string; nome: string; cor: string; desc: string; treino: string;
      pctMax: number; fcMin: number; fcMax: number; fcKarvMin: number; fcKarvMax: number;
    }[];
  }>(null);
      ...
      <main style={{ background: "#0D1117", minHeight: "100vh" }}>
        ...
        <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
          ...
          <div className="rounded-2xl p-6 space-y-5" style={{ background: "#161B22", border: "1px solid rgba(255,107,0,0.12)" }}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <input type="number" min="10" max="100" placeholder="Ex: 32" value={idade}
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {[20, 25, 30, 35, 40, 45, 50].map(v => (
                    <button key={v} onClick={() => setIdade(String(v))}
                      ...
                      style={{
                        background: idade === String(v) ? "rgba(255,107,0,0.2)" : "#21262D",
                        color: idade === String(v) ? "#FF6B00" : "#8B949E",
                        fontFamily: "'Barlow Condensed', sans-serif",
                        border: "1px solid " + (idade === String(v) ? "rgba(255,107,0,0.4)" : "transparent"),
                      }}>
                      {v}
                    </button>
                  ))}
              ...
              <div>
                ...
                <input type="number" min="30" max="100" placeholder="Ex: 55 (para Karvonen)" value={fcRepouso}
                  onChange={e => setFcRepouso(e.target.value)}
                  style={{ ...inp, border: "1px solid rgba(255,107,0,0.25)" }} />
                <p className="mt-1.5 text-xs" style={{ color: "#8B949E" }}>
                  Meça pela manhã ao acordar. Ativa o método Karvonen (mais preciso).
                </p>
              </div>
            </div>
              ...
              <div className="grid gap-2">
                {FORMULAS.map(f => (
                  <button key={f.id} onClick={() => setFormula(f.id)}
                    ...
                    style={{
                      background: formula === f.id ? "rgba(255,107,0,0.1)" : "#21262D",
                      border: "1px solid " + (formula === f.id ? "rgba(255,107,0,0.4)" : "transparent"),
                    }}>
                    <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full"
                      style={{ border: "2px solid " + (formula === f.id ? "#FF6B00" : "#8B949E") }}>
                      {formula === f.id && <div className="h-2.5 w-2.5 rounded-full" style={{ background: "#FF6B00" }} />}
                    </div>
                    <span className="text-sm font-bold"
                      style={{ color: formula === f.id ? "#E6EDF3" : "#8B949E", fontFamily: "'Barlow Condensed', sans-serif" }}>
                      {f.label}
                    </span>
                  </button>
                ))}
          ...
          {resultado && (
            <div className="space-y-4 animate-slide-up">
              ...
              <div className="rounded-2xl p-5 text-center"
                style={{ background: "linear-gradient(135deg, #2d1000, #1a0a00)", border: "1px solid rgba(255,107,0,0.3)" }}>
                <p className="text-xs font-black"
                  style={{ color: "rgba(255,107,0,0.6)", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.1em" }}>
                  SUA FREQUÊNCIA CARDÍACA MÁXIMA
                </p>
                <p className="text-7xl font-black" style={{ color: "#FF6B00", fontFamily: "'Barlow Condensed', sans-serif" }}>
                  {resultado.fcMax}
                </p>
                ...
                {resultado.fcReserva && (
                  <p className="mt-2 text-sm" style={{ color: "#8B949E" }}>
                    FC de Reserva:{" "}
                    <span style={{ color: "#FFB800", fontWeight: 700 }}>{resultado.fcReserva} bpm</span>
                  </p>
                )}
              </div>
              ...
              <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,107,0,0.15)" }}>
                ...
                <div>
                  {resultado.zonas.map((z, i) => (
                    <div key={z.zona} className="p-4"
                      style={{
                        background: i % 2 === 0 ? "#161B22" : "#1a1f26",
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl font-black"
                          ...
                          <span className="text-xs"
                            style={{ color: z.cor, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.05em" }}>
                            {z.zona}
                          </span>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            <span className="rounded-lg px-2 py-0.5 text-xs font-black"
                              style={{ background: z.cor + "20", color: z.cor, fontFamily: "'Barlow Condensed', sans-serif" }}>
                              {resultado.fcReserva
                                ? (z.fcKarvMin + "–" + z.fcKarvMax + " bpm")
                                : (z.fcMin + "–" + z.fcMax + " bpm")}
                            </span>
                          </div>
                          ...
                          {/* Barra visual — componente externo evita template literal em style */}
              ...
              <div className="rounded-2xl p-4" style={{ background: "#161B22", border: "1px solid rgba(255,184,0,0.15)" }}>
                <p className="text-xs font-black mb-2"
                  style={{ color: "#FFB800", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.08em" }}>
                  💡 COMO USAR
                </p>
                <p className="text-xs" style={{ color: "#8B949E" }}>
                  Para iniciantes: 80% do volume em Z1-Z2. Para intermediários: 70% em Z2, 20% em Z4, 10% em Z5.
                  Monitore com um relógio de FC para treinos mais precisos.
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Link href="/planos-treino" className="rounded-xl p-4 transition-all hover:-translate-y-0.5"
