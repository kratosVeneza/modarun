...
type Treino = {
  ...
  local_saida?: string;
  ponto_encontro_lat?: number;
  ponto_encontro_lng?: number;
...
export default function TreinoPage() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      // 3. Verifica se é admin pela tabela admins
      ...
      // Dono: user_id bate com uid, ou user_id é null (treino antigo)
      ...
      <>
        ...
        <main className="min-h-screen bg-slate-50 px-4 py-8">
          <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-3 font-semibold text-slate-700">Treino não encontrado</p>
          </div>
        </main>
      </>
    );
    ...
    <>
      ...
      <main className="min-h-screen bg-slate-50 px-4 py-8">
        <div className="mx-auto max-w-3xl space-y-5">
          ...
          {/* Hero */}
          <section className="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-orange-500 to-amber-500 p-7 text-white shadow-xl">
            <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-white/10" />
            <div className="relative">
              <div className="flex flex-wrap items-center gap-2">
                {treino.tipo_treino && (
                  <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">{treino.tipo_treino}</span>
                )}
                <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">
                  {participantes.length} participante{participantes.length !== 1 ? "s" : ""}
                </span>
                {isAdmin && (
                  <span className="rounded-full bg-white/30 px-3 py-1 text-xs font-bold">👑 Admin</span>
                )}
              </div>
              <h1 className="mt-3 text-2xl font-bold sm:text-3xl">{treino.titulo}</h1>
          ...
          {/* Botão excluir */}
          {podeExcluir && (
            <div className="flex items-center justify-between rounded-2xl border border-red-100 bg-red-50 px-5 py-3">
              <div>
                <p className="text-sm font-semibold text-red-700">
                  {isAdmin && treino.user_id !== userId ? "👑 Você é administrador" : "✏️ Você criou este treino"}
                </p>
                <p className="text-xs text-red-400">Somente você pode excluí-lo.</p>
              </div>
              <BotaoExcluirInline encontroId={treino.id} titulo={treino.titulo} />
            </div>
          )}

          {/* Info grid */}
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <InfoCard label="Data" value={formatarData(String(treino.data_encontro))} icon="📅" />
            <InfoCard label="Horário" value={String(treino.horario || "—")} icon="⏰" />
            <InfoCard label="Distância" value={treino.distancia || (treino.km_planejado ? `${treino.km_planejado} km` : "—")} icon="📏" />
            <InfoCard label="Organizador" value={treino.organizador_nome || "—"} icon="👤" />
          </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {treino.ponto_encontro_lat && treino.ponto_encontro_lng && (
                <p className="mt-1 text-xs text-slate-400">
                  {Number(treino.ponto_encontro_lat).toFixed(5)}, {Number(treino.ponto_encontro_lng).toFixed(5)}
                </p>
              )}
            </section>
          )}
          ...
          {(treino.ritmo || treino.percurso || treino.observacoes) && (
            <section className="space-y-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {treino.ritmo && (
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Ritmo</p>
                  <p className="mt-0.5 text-sm text-slate-700">{treino.ritmo}</p>
                </div>
              )}
              {treino.percurso && (
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Percurso</p>
                  <p className="mt-0.5 text-sm text-slate-700">{treino.percurso}</p>
                </div>
              )}
              {treino.observacoes && (
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Observações</p>
                  <p className="mt-0.5 text-sm text-slate-700">{treino.observacoes}</p>
                </div>
              )}
            </section>
          )}
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {participantes.length === 0 ? (
              <p className="text-sm text-slate-500">Seja o primeiro a confirmar presença!</p>
            ) : (
          ...
          <section className="rounded-3xl bg-gradient-to-r from-slate-900 to-slate-800 p-6 text-white shadow-lg">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <a href="https://wa.me/5594920009526?text=Olá! Vou participar de um treino pelo app Moda Run e quero ver produtos indicados." target="_blank" rel="noreferrer"
                className="rounded-2xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-orange-600">
                💬 WhatsApp
              </a>
              <a href="/loja" className="rounded-2xl border border-white/20 bg-white/10 px-4 py-2.5 text-sm font-semibold text-white hover:bg-white/20">
                Ver loja
              </a>
            </div>
          </section>
...
function BotaoExcluirInline({ encontroId, titulo }: { encontroId: number; titulo: string }) {
  ...
  async function excluir() {
    ...
    try {
      ...
      if (!response.ok) { setErro(result.error || "Erro ao excluir."); setLoading(false); return; }
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm"
        ...
        <div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
          <div className="mb-1 flex h-12 w-12 items-center justify-center rounded-2xl bg-red-100 text-2xl">🗑️</div>
          <h3 className="mt-3 text-lg font-bold text-slate-900">Excluir treino?</h3>
          <p className="mt-1 text-sm text-slate-500">
            O treino <span className="font-semibold">"{titulo}"</span> e todos os participantes serão removidos permanentemente.
          </p>
  ...
  return (
    <button onClick={() => setConfirmando(true)}
      className="flex items-center gap-1.5 rounded-2xl border border-red-200 bg-white px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50">
      🗑️ Excluir
    </button>
  );
}
