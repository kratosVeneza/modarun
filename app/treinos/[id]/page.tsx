        ...
        <div className="mx-auto max-w-3xl space-y-5">
          <section className="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-orange-500 to-amber-500 p-7 text-white shadow-xl">
            ...
            <div className="relative">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">{participantes.length} participante{participantes.length !== 1 ? "s" : ""}</span>
                {isAdmin && <span className="rounded-full bg-white/30 px-3 py-1 text-xs font-bold">👑 Admin</span>}
              </div>
              ...
              <div className="mt-5"><CompartilharTreino url={url} /></div>
            </div>
          </section>
          ...
          {podeExcluir && (
            <div className="flex items-center justify-between rounded-2xl border border-red-100 bg-red-50 px-5 py-3">
              <div>
                <p className="text-sm font-semibold text-red-700">{isAdmin && treino.user_id !== userId ? "👑 Você é administrador" : "✏️ Você criou este treino"}</p>
