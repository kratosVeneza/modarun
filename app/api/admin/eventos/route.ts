...
export async function PATCH(request: Request): Promise<NextResponse> {
  ...
  const { data, error } = await supabase.from("eventos").update({ nome, cidade, estado, data_evento, distancia: distancia || null, local: local || null, link_inscricao: link_inscricao || null, destaque: Boolean(destaque) }).eq("id", id).select();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
