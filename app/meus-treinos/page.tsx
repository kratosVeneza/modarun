...
import BotaoExcluirMeusEncontros from "@/components/BotaoExcluirMeusEncontros";
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {isAdmin ? `Visão de administrador · ${encontros?.length || 0} treino${(encontros?.length||0)!==1?"s":""} no total` : `Treinos que você organizou · ${encontros?.length || 0} no total`}
                </p>
              </div>
                ...
                <div key={e.id} className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-lg">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2">
                      ...
                      <BotaoExcluirMeusEncontros encontroId={e.id} titulo={e.titulo} />
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {[["Data",formatarData(String(e.data_encontro))],["Horário",String(e.horario||"—")],["KM",e.km_planejado?`${e.km_planejado} km`:(e.distancia||"—")],["Participantes",String(participantes)]].map(([l,v],i)=>(
